...
const filterStringsWithVowels = (arr) => {
  // let regex = /[a-zA-Z]*[aeiou][a-zA-Z]*/gi;
  let regex = /[aeiou]/;

  return arr.filter(hasVowels => regex.test(hasVowels));
}
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(allowedWords => !forbiddenValues.includes(allowedWords))
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let stat = arr.filter(baseStat => {
    baseStat.baseStat > minBaseStat});
  return stat;
};
